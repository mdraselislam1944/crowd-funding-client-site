...
export const createUser = createAsyncThunk(
    ...
    async ({ email, password, name }, { rejectWithValue }) => {
        try {
            ...
            return rejectWithValue({ message: 'Error creating user', originalError: error });
        }
    }
);
...
export const logoutUser = createAsyncThunk(
    ...
    async (_, { rejectWithValue }) => {
        try {
            ...
            console.error("error message is: ", error.message);
            return rejectWithValue({ message: 'Error creating logout user', originalError: error });
        }
    }
);
...
export const loggedUser = createAsyncThunk('userSlice/loggedUser', async ({ email, password }, { rejectWithValue }) => {
    try {
        ...
        return rejectWithValue({ message: 'Error loggedIn user', originalError: error });
    }
}
);
const googleProvider = new GoogleAuthProvider();
export const loggedWithGoogle = createAsyncThunk('userSlice/loggedGoogleUser', async (_, { rejectWithValue }) => {
    try {
...
const userSlice = createSlice({
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(loggedWithGoogle.pending, (state) => {
                state.isLoading = true;
                state.isError = false;
                state.email = "";
                state.name = "";
            ...
            .addCase(loggedWithGoogle.fulfilled, (state, { payload }) => {
                state.isLoading = false;
                state.isError = false;
                state.email = payload.email;
                state.name = payload.name;
            ...
            .addCase(loggedWithGoogle.rejected, (state, action) => {
                state.isLoading = false;
                state.isError = true;
                state.error = action.payload.message;
            });
